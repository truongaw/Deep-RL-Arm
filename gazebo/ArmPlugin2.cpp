...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		if ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_POINT) == 0)||(strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0))
...
bool ArmPlugin::updateAgent()
{
	...
	/
	*/
	uint32_t index = action/2;
	float velocity = vel[index];
// TODO - Set joint velocity based on whether action is even or odd.
	...
	*/
	uint32_t index = (action)/2;
	float joint = ref[index]; // TODO - Set joint position based on whether action is even or odd.
	if (action %2){
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( maxEpisodeLength > 0 && episodeFrames > maxEpisodeLength )
	{
		...
		rewardHistory = REWARD_LOSS/2.0f;
		newReward     = true;
		endEpisode    = true;
	}
